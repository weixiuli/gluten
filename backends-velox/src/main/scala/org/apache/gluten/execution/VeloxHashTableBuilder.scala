...
import org.apache.gluten.vectorized.{ColumnarBatchSerializeResult, VeloxHashTableJniWrapper}
...
object VeloxHashTableBuilder {
  case class SerializedHashTable(id: Long, bytes: Array[Byte])

  private val idGenerator = new AtomicLong(0L)

  private def nextHashTableId(): Long = idGenerator.incrementAndGet()

  def build(
      output: Seq[Attribute],
      mode: BroadcastMode,
      serialized: Array[ColumnarBatchSerializeResult]): Option[SerializedHashTable] = {
    ...
    try {
      ...
      if (bytes != null && bytes.nonEmpty) {
        Some(SerializedHashTable(nextHashTableId(), bytes))
      } else {
        None
      }
    } finally {
